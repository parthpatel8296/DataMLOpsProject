...
def run_batch_ingestion(ingestion_date_str=None):
    ...
    source_path = PROJECT_ROOT / "data" / "source" / "user_interactions" / f"{ingestion_date_str}_user_interactions.csv"
    ...
    try:
        ...
        spark = SparkSession.builder \
            .appName("DailyUserInteractionBatchIngestion") \
            .getOrCreate()
        ...
        output_path = PROJECT_ROOT / "data/raw/batch/user_interactions" / f"ingestion_date={ingestion_date_str}"
        print(f"Writing to: {output_path}")
        Path(output_path).mkdir(parents=True, exist_ok=True)
        df.write.mode("overwrite").parquet(str(output_path))

        logging.info(
            f"User Interactions Batch ingestion successful | rows={df.count()} | path={output_path}"
        )

        print("User Interactions Batch ingestion completed successfully.")

    except Exception as e:
        print("ERROR occurred:", e)
        logging.error(f"User Interactions Batch ingestion failed: {e}")
        raise e # Re-raise to ensure calling process knows it failed
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run User Interactions Batch Ingestion")
